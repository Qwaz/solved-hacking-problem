...
for bin_id in arch_map[Arch.ARM32]:
    ...
    if (
        inst_map[0x10420].mnemonic != "add"
        or inst_map[0x10424].mnemonic != "bl"
        or inst_map[0x1042C].mnemonic != "bne"
    ):
        continue

    find = parse_imm(inst_map[0x1042C].op_str)
    avoid = [0x10430]
    ...
    state = proj.factory.blank_state(
        addr=0x10420, add_options={angr.options.ZERO_FILL_UNCONSTRAINED_REGISTERS}
    )
