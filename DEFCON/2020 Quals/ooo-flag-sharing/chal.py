...
def reconstitute_secret(keys, matrix):
    ...
    detTinv = [ det * inv_float[0][i] for i in range(k) ]
    rounded = [ int(round(h)) for h in detTinv ]
    zipped = [ i*pow(det, -1, P) for i in rounded ]

    s = sum([ i*j for i,j in zip(zipped, subshares) ])
    key = (int(s) % P).to_bytes(32, byteorder='little')
